...
# Hidden GPS injection field (read-only)
gps = st.text_input("📍 GPS Location", key="gps-data", label_visibility="collapsed", disabled=True)

# Centered share button + JS
with st.container():
...
# Wait until location is captured
if not gps:
    st.warning("📍 Click the button above to share your location.")
    st.stop()
