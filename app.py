...
if distance > allowed_distance:
    st.error(f"❌ You are {int(distance)} meters away. Must be within 100 meters to proceed.")
    st.stop()
...
with st.form("attendance_form"):
    ...
    action = st.radio("Choose Action", ["Punch In", "Punch Out"], disabled=(punch_in_done and punch_out_done or (punch_in_done and not punch_out_done and "Punch In" in records_today["Action"].values)))
